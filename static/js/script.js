...
document.addEventListener('DOMContentLoaded', function() {
    const jefeItems = document.querySelectorAll('.jefe-item');
    const jefeIdInput = document.getElementById('jefeIdInput');
    const jefeSeleccionadoText = document.getElementById('jefeSeleccionadoText');
    const sucursalSelect = document.getElementById('sucursalSelect');
    const confirmModal = document.getElementById('confirmModal');
    const modalConfirmBody = document.getElementById('modalConfirmBody');
    const confirmButton = document.getElementById('confirmButton');
    const asignarSucursalForm = document.getElementById('asignarSucursalForm');

    let jefeSeleccionado = null;

    // Manejar selección de jefes
    jefeItems.forEach(item => {
        item.addEventListener('click', function(e) {
            e.preventDefault();
            
            // Remover clase active de todos
            jefeItems.forEach(i => i.classList.remove('active-custom'));
            ...
            jefeSeleccionado = {
                id: this.getAttribute('data-jefe-id'),
                nombre: this.querySelector('div').textContent.trim()
            };
            
            jefeIdInput.value = jefeSeleccionado.id;
            jefeSeleccionadoText.textContent = jefeSeleccionado.nombre;
        });
    });

    // Manejar confirmación
    confirmButton.addEventListener('click', function() {
        if (jefeSeleccionado && sucursalSelect.value) {
            asignarSucursalForm.submit();
        }
    });
    ...
    confirmModal.addEventListener('show.bs.modal', function(event) {
        if (!jefeSeleccionado || !sucursalSelect.value) {
            ...
            alert('Por favor seleccione un jefe y una sucursal');
            return;
        }

        const sucursalSeleccionada = sucursalSelect.options[sucursalSelect.selectedIndex].text;
        modalConfirmBody.innerHTML = `
            ¿Está seguro de asignar <strong>${sucursalSeleccionada}</strong> 
            a <strong>${jefeSeleccionado.nombre}</strong>?
        `;
    });
});
